...
- **Ratios** (`R_probe`, `R_free`): ratio of medians — `median` of numerators over `median` of denominators, seeds stacked as draws, `moved` windows only — with the 95 % episode-bootstrap interval `pool_ratio` implements (2000 draws, seed 0), uncorrected as in the ladder. (added after the run, 2026-09-13) `pool_ratio` normalises each cell by its own denominator median before stacking, so the pooled estimand is `median(num_c / med(den_c)) / median(den_c / med(den_c))` over the stacked draws, not the median of the raw stacked numerators over the median of the raw stacked denominators; the two coincide only when the cells' denominator medians agree. On the shipped run the spec-literal stacked values are `R_probe` 0.993 / 0.873 / 0.827 and `R_free` 0.189 / 0.257 / 0.210 (`pixel_ae` / `frozen_ssl` / `random_vit`) against the tool's 1.019 / 0.861 / 0.797 and 0.169 / 0.266 / 0.209; the argmin is the same arm under either estimand on both channels.
...
- **Measurability.** A cell enters the probe-based pooling iff its persistence-to-floor band at h=45 is positive — the ladder's `probe_is_measurable` rule; all nine shipped cells pass it. (added after the run, 2026-09-13) The rule governs Reading 2's probe channel as well: its `S(h)` and `H*_q` through the probe pool the measurable cells only, the free channel every cell, and `trust.txt`'s pooling notes list the cells excluded from it. A second line recomputes every probe-based statistic with cells of probe selection R² < 0.1 excluded — a threshold chosen here knowing `pixel_ae`/s1 reads 0.018 and every other cell ≥ 0.249 — as a **sensitivity report that changes no verdict**.
...
- **(iii) Take the magnitude out and the gate ranking goes away.** On each fold, `paired_contrast(frozen_ssl − random_vit)` on the held-out `c_w(45)`: holds iff `z < −z_fam` (the treatment's corrected error is smaller); fails iff `z > z_fam`; *undecided* otherwise. (iii) holds only if it holds on both folds, and is *unreadable* — Reading 1 *unresolved* — if either arm's α is on the grid boundary on either fold. (added after the run, 2026-09-13) Pooled over seeds, "either arm's α is on the grid boundary" means a boundary α at h=45, on either fold, in any measurable seed of that arm — the pooled held-out contrast pooled those seeds, and a correction that is persistence itself in one of them is not a correction; the per-seed evaluation reads each seed's own flag.
