...
Nine of these records are the entire output of a 13.5-GPU-hour study, so every
test here is about a field that is silently wrong rather than absent.

SPLIT BY CONCERN. This module holds the record itself -- its path, its
contents, the NaN policy and the sanitiser's corners, reproducibility and the
held-out split, the checkpoint guards, and the values and the protocol the
nine cells actually run at -- together with the shared job fixture and the
pairwise-distinct parameters every sibling imports. Two concerns live in
their own modules: `test_study_reward.py` (reward accuracy on a near-constant
target) and `test_study_provenance.py` (`git_sha`, `device`,
`encoder_params` and the full `history`, M3c).
"""
...
def test_the_fixture_parameters_are_pairwise_distinct_so_no_assertion_is_vacuous():
    ...
    # to run, they ARE equal -- `random_vit`'s cache is called `random_vit`
    # and `pixel_ae`'s is called `pixel_ae` -- so `backbone = job.arm` was a
    # numerical no-op and survived the whole suite. In the study
...
def test_the_jobs_device_reaches_the_training_config_and_the_evaluation(
    ...
    13.5-hour loss. Run at a device string that is NOT the one a mutation would
...
def test_the_out_dir_and_the_record_path_accept_the_strings_argparse_gives(
    ...
    the driver's very first action:
      ...
      * `job_record_path`: `Path(out_dir) / ...` -> `out_dir / ...`. THIS IS
        THE RESUME PATH. The driver calls it to decide whether a ~1.5-hour
