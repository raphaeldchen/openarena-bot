...
### M1 — Data pipeline
Random-policy collection, episode storage with eviction, sequence loader, frozen-feature cache.

**Done when:** a target transition count is collected within a bounded wall-clock time; the loader's
batches/sec is benchmarked; a round-trip test proves stored data equals collected data; feature cache
produces byte-identical features on repeat encoding of the same frame.
